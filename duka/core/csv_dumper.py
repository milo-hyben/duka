...
TEMPLATE_FILE_NAME = "{}-{}_{:02d}_{:02d}-{}_{:02d}_{:02d}-{}"
...
class CSVDumper:
	def __init__(self, symbol, timeframes, start, end, folder, header=False, local_time=False):
		self.symbol = symbol
	...
	def dumpFeather(self):
		...
		for timeframe in self.timeframes:
			...
			for day in sorted(self.buffers[timeframe].keys()):
				if df is False:
					df = pd.DataFrame(self.buffers[timeframe][day], columns=['date', 'ask', 'bid', 'askVol', 'bidVol'])
					#df = pd.DataFrame.from_records(data=self.buffers[timeframe][day], columns=['date', 'ask', 'bid', 'askVol', 'bidVol'], index=[0])
					#print(df.dtypes)
				else:
					#print(self.buffers[timeframe][day])
					df.append(self.buffers[timeframe][day])

			df.reset_index(drop=True, inplace=True)
			#print(df.dtypes)
			df.to_feather(join(self.folder, file_name + '.feather'))
			df.to_csv(join(self.folder, file_name + '.csv2'))
	...
	def dump(self):
		...
		for timeframe in self.timeframes:
			...
			with open(join(self.folder, file_name + '.csv'), 'w', newline='\n') as csv_file:
